...
async function startYamlGenerator() {
  ...
  await generateYamlFilesForWsdl(`${__dirname}/wsdl/nodo/CdPerNodo.wsdl`);
  await generateYamlFilesForWsdl(`${__dirname}/wsdl/nodo/NodoPerPsp.wsdl`);
...
async function generateYamlFilesForWsdl(wsdlPath) {
  ...
  for (const item in serviceData.services) {
    ...
    fs.writeFileSync(`./yaml-file/${serviceName}.yaml`, yaml.safeDump(swagger));
  }
}
